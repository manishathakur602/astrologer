...
const TestimonialsSection = () => {
  ...
  return (
    <section className="bg-gradient-to-br from-[#fff6e5] to-white py-20 px-4">
      <div className="max-w-7xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {paginatedTestimonials.map((testimonial, index) => (
            <div
              key={index}
              className="bg-[#390C05] backdrop-blur-lg border border-[#FFC107]/30 rounded-3xl shadow-lg p-8 transition-transform transform hover:scale-105 hover:shadow-2xl duration-300"
            >
          ...
          {Array.from({ length: totalPages }).map((_, index) => (
            <div
              ...
              className={`w-4 h-4 rounded-full cursor-pointer transition-all duration-300 ${
                index === pageIndex ? 'bg-[#390C05]' : 'bg-gray-300'
              }`}
